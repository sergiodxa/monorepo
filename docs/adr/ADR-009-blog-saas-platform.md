...
- **auth-saas is the critical-path dependency**: it must be deployed with the `sso.blog.sergiodxa.com` tenant, a dashboard client, and an M2M management client before Phases 4+ can ship. The plan is for `auth.sergiodxa.com` itself to become the auth-saas platform, using email + passkey with magic-link recovery and Cloudflare Email Sending instead of Resend — that work is tracked under ADR-006, not here. While porting its patterns, fix the `custom_metadata` casing mismatch (`tenant_id` written vs `tenantId` read) that exists in auth-saas today.
