...
| 10  | **A billing outage refuses a sign-in.** A first social sign-in provisions the subject, the connection and the Polar customer in one `Promise.all`; a refused Polar call throws out of the strategy and the person is turned away, while the database writes it raced with may already have committed. Nobody is charged at sign-up, so this puts the identity server's availability behind a billing vendor's for an operation with no money in it | `app/strategies/github.ts`                                     | Medium — a billing outage becomes a total signup outage |
...
| 9   | **Fixed: registration stores the credential verified; the refusal stays for every other way a credential can appear.** What `credentials.verified_at` can mean here was settled by what writes it and what reads it: nothing writes it (no verification email, no token table, no route — in either app), and one place reads it, refusing sign-in when it is `null`. A column no code can ever set is not a "not yet"; it is a permanent refusal, which is why password sign-up in the app being ported was dead code and why the production table holds 0 rows. It is therefore read as **"this password is known to belong to whoever holds this account"**, not as email ownership — `subjects.email_verified_at` is the email question, is untouched by this decision, and stays `null` for a credential registration, so `email_verified` in userinfo keeps telling relying parties the truth. Registration (an address nobody had registered) passes `new Date()`: the person chose the password for an account that did not exist a moment earlier, so there is no other owner to protect it from, and refusing them only reproduces the defect. The other branch — a **known** address with no credential, i.e. somebody who signs in with GitHub — still stores the hash with `null` and still fails with `MissingValidationError`, because there the account belongs to someone else and a stranger who knows their address must not be able to attach a working password to it; marking that credential verified would turn this endpoint into account takeover. That is the reason the gate exists and it is not weakened. Accepted cost: a registration that succeeds is distinguishable from a wrong password, so the endpoint reveals whether an address is registered — inherent to any working sign-up, and the GitHub callback already says so out loud ("An account already exists for this email address"). Alternative rejected at port time: inventing a verification-email flow, which is a product feature with a mail dependency, a token store and a route, not a port decision. That flow was built afterwards (§17); it did not weaken this refusal and did not touch this column |
| 10  | **Changed deliberately: the billing mirror is best effort and never fails a sign-in.** The NEW APP creates the subject, then the connection, then the Polar customer. Only the connection keeps its compensation — a subject with no connection cannot sign in and still holds the address on the unique column, so the next attempt would collide with a row nobody can reach — and a failed Polar call is logged as `github_customer_create_failed` (event name plus subject id) and otherwise ignored. Rolling the sign-up back on a billing failure was the NEW APP's first shape and is rejected: it refuses a person for an outage they cannot act on, erases the account it just refused so a retry re-runs the whole provisioning against the same outage, and is not even sound — `Customer.findOrCreateByEmail` creates the customer and links it in a second call, so a failure between the two leaves an orphaned Polar customer whose external id names a subject that no longer exists. Nothing is shown to the person: they signed in, and there is nothing for them to do. Reconciliation is left as a follow-up rather than built now, because a subject with no Polar customer is already an ordinary state — credential registration has never created one on either app — and `findOrCreateByEmail` is idempotent, so the gap is repaired by any later run against the same subject. Until something automates it, repair is: list the affected subject ids from the `github_customer_create_failed` log lines, and for each one call `Customer.findOrCreateByEmail(polar, subject.email_address, subject)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`POST /authorize` and the GitHub callback — call it unconditionally on success and let it
decide. It is deliberately one condition rather than two rules to reconcile: a message is
only needed when the person signed in with a password credential, or when the provider did
not report the address as verified, and a provider that _did_ report it verified leaves the
column stamped. So the null check subsumes the method check, and neither call site knows how
anybody signed in. A registration takes the same path, which is what gives a brand-new
password account its first message. **Nothing is sent on a refused sign-in**: both calls sit
after the success branch has been taken, because an endpoint that mails whichever address was
typed at it is an account-existence oracle with a mailer attached. SSO sends nothing either,
for the reason it sends no sign-in alert.

**The token.** `randomToken({ bytes: 32 })` from `@pkg/crypto` — 256 bits, base64url. KV holds
`email-verification:<sha256hex(token)>` → `{ subjectId, emailAddress }` with a 5-minute
`expirationTtl`. The key is the digest, not the token, so reading the namespace yields hashes
rather than links somebody could follow. It is single-use because consumption deletes the
record before anything is written, the same way an authorization code is consumed. Expiry is
KV dropping the record on its own, so an expired token and a replayed one are the same
condition and get the same page. The record binds **both** the subject and the address: a
token whose stored address is not the row's current address is refused rather than applied, so
a token cannot confirm an address it was not mailed to. Every refusal — expired, replayed,
malformed, address moved — renders one page, because they are one outcome to the reader and
distinguishing them tells whoever holds the link something they should not learn from it.

**The send window, which is not optional.** "Send whenever the column is null" on a sign-in
endpoint is a mail-bombing amplifier: anyone who can drive the form for an unverified address
can make this server send mail on demand, against the sending domain's reputation and the
account's daily quota. The window is per address, keyed as
`email-verification-cooldown:<sha256hex(lowercased address)>` so no key spells out an address,
and it is claimed only after the token has been stored — a window opened for a token that
failed to store would suppress sends for a link nobody can use.

**The invariant is `cooldown ≤ token TTL`.** Because a held-back send can only ever have
replaced a token that is still usable, suppression cannot strand a person with no way to
verify: someone who asks again inside the window already holds a working link, and someone who
waits past it gets a fresh message. This flow satisfies it by equality — one
`VERIFICATION_TTL_MS` constant is read for both, so they cannot drift, and 5 minutes for the
window is the shortest that still bounds mail volume usefully. The password-reset flow
satisfies the same inequality with a 30-minute token and a 5-minute window. Changing either
...
would answer a good token with a sign-in page. The token is what authorizes the write, which
is why it is unguessable, short-lived, single-use and bound to both the subject and the
address.

**Email verification does not gate sign-in, and `credentials.verified_at` is untouched by
this.** The two columns answer different questions — the credential one is "is this password
known to belong to this account", the subject one is "is this address confirmed" — and
conflating them is what would reproduce defect #9. Gating sign-in on
`subjects.email_verified_at` would lock a person out of the only place that can resend the
message, which is the failure mode this flow exists to avoid, and it would put the gate in the
wrong layer: an identity provider's job is to publish `email_verified` honestly and let each
relying party decide what it will accept. So an unverified subject signs in, sees the state on
their profile and can act on it, and every relying party is told the truth about the address.
Defect #9's refusal is unchanged: a password submitted for an address that already has an
account with no credential still stores the hash unverified and still fails, because there the
account belongs to somebody else.

**The signal.** `/account/profile` renders an `Unverified` badge beside the address and, while
it is unverified, an `Alert` panel saying what that costs and posting the resend. The resend
answers with a redirect naming its outcome in the query (`?resend=sent|suppressed|failed`),
which the view model validates against the outcomes it has a sentence for so a hand-typed
value renders nothing. Suppression is worded as "already sent, the link still works" rather
than as a refusal, which is what the inequality above makes true.

**Logging.** Event names plus the subject id: `email_verification_queued`,
`email_verification_suppressed`, `email_verification_failed`,
`email_verification_subject_missing`, `email_verification_resend_requested`,
`email_verification_token_malformed`, `email_verification_token_refused`, `email_verified`. No
token, no digest of one, no address — a log line naming any of those is a log line somebody
could verify an address with.

**The provider-verification gap is fixed.** `resolveGitHubSubject` used to write
`email_verified_at: Date.now()` on a GitHub first sign-in, asserting that GitHub only releases
verified addresses. It does not, and the code path could not have known either way:
`remix/auth`'s GitHub provider reads `/user/emails` only when the profile's own `email` is
null, and its `pickGitHubEmail` falls back to `emails[0]` when no entry is verified, so the
per-address `verified` boolean is dropped before the profile reaches this app and
`GitHubAuthProfile` carries no verification signal at all — verified in the provider's source.
This server therefore recorded a verification it never observed and published it as
`email_verified: true`. `app/services/github-login.ts` now reads `/user/emails` itself with the
token the callback exchanged (`user:email` is already among the requested scopes), matches the
profile's address case-insensitively against an entry marked `verified`, and threads that
boolean to `Subject.create`. It fails closed in every direction — no address, a refused or
unreadable response, an address the list does not contain — because a verification nobody
observed is worse than none.

**An address GitHub reports unverified is still usable for a first sign-in, and lands
unverified.** Refusing it was considered and rejected: it would leave the person no way in
from this server at all, while allowing it costs nothing that `email_verified: false` does not
already tell every relying party, and it puts the account straight into the flow that asks
them to prove the address. The existing refusal that does the protecting is unchanged — a
first GitHub sign-in whose address already belongs to a subject is still `access_denied`
rather than linked. The residual risk is narrow and worth writing down: somebody whose GitHub
account has no verified address at all can provision a subject holding an address they do not
own, which claims the unique `email_address` column and produces an `email` claim that a
relying party ignoring `email_verified` would trust. Both halves of that are pre-existing
properties of keying on a provider-supplied address; what changed is that the claim is now
accompanied by an honest `email_verified: false`.

#### The password-reset flow

Built on the same hooks. Two `form()` routes — `/password/forgot` and `/password/reset` — plus
`app/services/password-reset.ts` (the token store and the cooldown), `app/emails/reset-password.tsx`,
`app/emails/password-changed.tsx`, and one `Credential` method. There is no schema change: the
frozen database has nowhere to put a pending reset, so the whole store is KV.

**An unknown address is answered identically, and that is the endpoint's design rather than a
property of it.** The controller cannot branch on whether the address is registered because
`requestPasswordReset` returns `void`: the lookup, the issue and the send all happen inside it and
nothing comes back out. Both cases run the same validation, claim the same per-address cooldown —
claimed **before** the lookup, so an unregistered address costs the same reads and is rate-limited
the same way — and render the same "check your inbox" page with the same `200` and the same headers.
Neither is reported: `password_reset_unknown_address` and `password_reset_suppressed` carry no
payload at all, and no log line on this path ever carries an address. The residual signal is
**timing**: `@pkg/mail`'s middleware flushes its `later()` queue in a `finally` around the handler,
so the response for a registered address is produced after a body render and a provider round trip
that the other branch does not perform. Closing that would mean sending outside the request
lifetime, which today means either the container's `Mailer` (which §17 forbids HTTP handlers from
resolving) or a queue this worker may not consume until Phase 8 — so it is documented, not fixed.

**Tokens.** 32 bytes from `@pkg/crypto`'s `randomToken`, stored under
`password-reset:<sha256-hex(token)>` so the namespace holds digests rather than credentials, with
the value `{ "subject_id": … }` — validated with `remix/data-schema` on read, so a record whose
shape ever drifts reads as "no such token" instead of resetting whatever parsed. The binding is
that value: the reset form carries no subject id, so there is nothing to tamper with. Single use is
a delete before the caller is told anything, which is the order that makes a concurrent second
submission find nothing. `password-reset-latest:<subjectId>` holds the newest digest so issuing a
token retires the previous one, leaving at most one live reset per account.

**TTL 30 minutes, cooldown 5 minutes.** The verification flow's 5 minutes is right for a code typed
back into the same tab and wrong for a link read in an inbox and opened on another device; an hour
is a bearer credential sitting readable in a mailbox and a notification mirror for twice as long as
it needs to. The invariant the verification flow expressed as equality is `cooldown ≤ TTL`, and it
holds here strictly: a request suppressed inside the window is one whose existing token is still
valid, so suppression can never strand somebody with no usable link — with the extra property that
waiting out a cooldown takes 5 minutes rather than 30. Raising the cooldown above the TTL would
break it, so the two still move together.

**The existing limiters are spent as well but are not the control.** Both endpoints spend the
login limiter (10/60s, IP-keyed through `getClientIP`), including the `GET` that opens a link. That
bounds request volume; only the per-address cooldown bounds mail volume, because a distributed
caller defeats an IP key.

**A completed reset revokes every session.** `sessions.id` _is_ the refresh token, so a session
surviving a reset is a live credential in exactly the hands the reset was performed against.
Back-channel logout tokens go out first — the recipient list is derived from the session rows, so
it cannot be computed after they are deleted — best effort and logged on failure, because an
unreachable relying party must not stop the refresh tokens from dying. The browser's own session
tokens are unset too, so the cookie stops claiming a sign-in that no longer resolves.

**A completed reset stamps `credentials.verified_at`, and creates the credential when the subject
had none.** The column means "this password is known to belong to this account" (§16 defect #9),
and somebody who read the account's inbox and spent a single-use token issued for that subject has
demonstrated precisely that — a stronger proof than registration's, which only establishes that
nobody had claimed the address yet. Leaving it null would store a hash sign-in refuses forever,
reproducing the defect at the end of the one flow whose purpose is ending it. This does **not**
weaken the gate defect #9 keeps: `POST /authorize` still refuses a password submitted for a known
address with no credential, because that caller has proved nothing. The reset path is the same
account reached by a caller who additionally proved inbox control, which is the whole difference,
and it is what finally gives that refused person a way forward. `subjects.email_verified_at` is
deliberately untouched: it answers a different question and it belongs to the verification flow.

**A password-changed notice is sent.** The reset mail itself can be deleted from a mailbox somebody
else is reading; this one arrives after the fact, states that the password changed and that
everything was signed out, carries no token and no link that acts on the account, and its reply-to
reaches a person. It is gated behind a spent token, so it needs no cooldown of its own.

**Not wired: a "forgot your password?" link on the sign-in page.** `resources/views/authorize.tsx`
was being edited concurrently, so the pages are reachable by URL and from the mail only. Adding
that link is the one thing this flow still needs for discoverability.
...
- [x] Phase 7: API, jobs, i18n sweep, docs
...
- [x] Email address verification and password recovery (§17, outside the numbered phases)
...
- [x] RP-initiated logout: expired `id_token_hint` (production hotfix, outside the numbered phases)

  **The defect predates the port.** `apps/auth` has the identical `IdToken.verify` call, so this was never a regression the port introduced and rolling back would not have fixed it. It reached production as a Cloudflare 1101 ("Worker threw exception") on `/oidc/logout`, and `wrangler tail` named it: `JWTExpired: "exp" claim timestamp check failed`, thrown out of `OIDC.logout`.

  **Two defects, both in `app/auth/oidc-provider.ts`, neither on the §16 list:**

  1. **An expired `id_token_hint` was refused, which is a spec violation.** `IdToken.verify` forwards its options straight to `jose.jwtVerify`, so every claim check ran — including `exp`. OpenID Connect RP-Initiated Logout 1.0 defines `id_token_hint` as a hint about which session is ending, not as a credential authenticating the request, and says the OP SHOULD accept an ID token even once its `exp` has passed. A relying party logging somebody out with a token from an earlier session is therefore the ordinary case, and it was the one case this endpoint could not serve. The hint is now verified with `clockTolerance: ID_TOKEN_HINT_CLOCK_TOLERANCE` (`Number.MAX_SAFE_INTEGER`), a named module constant whose JSDoc says why expiry is deliberately unchecked and that the exemption is the logout hint's alone. `clockTolerance` was chosen over `jose`'s `currentDate`: it can only ever widen a check, so it cannot turn a hint that would otherwise verify into a refusal, where evaluating the claims at an artificial instant would reject any token carrying `nbf`. The signature, the issuer and the `ES256` algorithm — the three things that actually make a hint trustworthy — are unchanged, and no other ID-token verification in this app was touched.
  2. **Any unusable hint became a 500.** The controller catches `OIDC.InvalidRequestError` and rethrows everything else, so a bad signature, a foreign issuer or an unparseable string escaped as a worker exception. Verification is now wrapped and every rejection becomes `InvalidRequestError("Invalid id_token_hint")`, which the controller already answers as `400 {"error":"invalid_request"}`. The underlying reason is deliberately not carried out or logged: it describes a token. `getSigningKey()` was lifted out of the wrapper so a key-store outage stays the fault it is rather than being reported as the client's mistake, and nothing is deleted before the refusal.

  **One more escape route closed.** `deliverBackchannelLogoutTokens` contains per-recipient failures in `Promise.allSettled`, but its `getSigningKey()` call sat outside that and could reject — and it runs _after_ the sessions are deleted, so it would have failed a sign-out that had already happened. It is now inside the same best-effort boundary, swallowed for the reason the settled results are. Judged safe and left alone: `postLogoutUrl` and `buildFrontchannelLogoutUrls` construct `URL`s from admin-registered columns, never from request input — `post_logout_redirect_uri` reaches them only after being compared for equality with the registered value.

  **Ordering preserved.** The recipient lists are still collected before the sessions are deleted, and an unregistered `post_logout_redirect_uri` is still refused before anything is deleted; both have tests asserting it.

  **8 new tests, all of which failed before the change** — the engine-level expired-hint test failed with the exact production message. In `app/auth/oidc-provider.test.ts`: "accepts an expired id_token_hint", "rejects an id_token_hint this server did not sign", "rejects an id_token_hint issued by somebody else", "rejects a malformed id_token_hint", "does not fail the logout when the back channel cannot be delivered". In `app/http/controllers/oidc/logout.test.ts`, which asserts the status code because the visible symptom was a 500: "logs out with an expired id_token_hint instead of failing" (303), "refuses an id_token_hint this server did not sign with a 400", "refuses a malformed id_token_hint with a 400". `bun typecheck`, `bun lint`, `bun run test` (9887 tests, 0 fail), `bun format:fix`, `bun run build` and `bunx wrangler deploy --dry-run` all pass; the dry-run still lists eleven bindings including the `POLAR_ACCESS_TOKEN` Secrets Store secret, and `dist/ssr/wrangler.json` still carries the `auth.sergiodxa.com` custom domain, the `0 0 * * *` cron and the `auth` queue consumer and producer.

- [x] RP-initiated logout: a `post_logout_redirect_uri` with no hint (production hotfix, outside the numbered phases)

  **The symptom.** `GET /oidc/logout?post_logout_redirect_uri=<a registered client's logout URI>` — no `id_token_hint`, no `client_id` — answered `400 {"error":"invalid_request"}` and, worse, left the person signed in: the refusal happened before a single session was deleted. This is the ordinary case for a relying party whose stored session no longer holds an ID token, so it has no hint to send.

  **The cause was the previous fix's guard, not its intent.** That guard resolved a client from `id_token_hint` or from an explicit `client_id`, then refused whenever a `post_logout_redirect_uri` was present and no client had resolved. It conflated two questions: _is this address registered_ (what the spec requires) and _did the request name the client that registered it_ (what the guard actually asked). Only the first is a security property.

  **1. The destination is now verified by lookup rather than by naming its client.** When no client resolved from the hint or `client_id`, `logout()` resolves one by exact match on the registered logout URI, through a new `findClientByLogoutUri` on `OIDC.Repository` — implemented in `app/auth/repository.ts` over `Client.findByLogoutUri`, a plain `db.findOne(clients, { where: { logout_uri }, orderBy: ["created_at", "asc"] })`. A hit means a registered client nominated that exact address, which is the whole property, so the redirect proceeds.

  **Why this does not reopen the open redirect.** The comparison is the same string equality the old guard used — `client.logoutUri !== args.postLogoutRedirectUri` became a `where logout_uri = ?` — with no prefix, origin, normalisation or trailing-slash leniency added anywhere. The set of addresses that can be redirected to is therefore unchanged: exactly the values administrators wrote into `clients.logout_uri`. What changed is only _which requests_ may reach an address in that set, never the set itself. An attacker who could previously not get `https://evil.example/` honored still cannot, because no row holds that string; the only way in remains registering a client, which is an admin action. Four repository tests pin the exactness (`/logout/`, the bare origin, a `..` segment, and a suffixed hostname `blog.example.com.evil.test` all resolve to `null`), and a router test asserts the `Location` header on an unregistered address is this server's own `/authorize` and does **not** contain the attacker's host.

  **Duplicate `logout_uri` registrations: answered, not refused.** Nothing in the schema stops two clients registering the same address, and it does not matter — the lookup asks whether the address is registered, and either row answers yes with equal authority. Refusing on ambiguity would break a legitimate logout for a reason that carries no security content. The oldest registration is returned so the answer is stable rather than row-order dependent, and — the part that actually matters — the client found this way is **never** promoted to the initiating `clientId`: it only verifies the address. Matching an address says nothing about who started the logout, and treating it as the initiator would silently drop a relying party from the back- and front-channel fan-out. A repository test registers two clients on one URI and asserts one of them comes back, the same one on every call; an engine test asserts `clientId` stays `undefined` and both fan-out queries are still called with no exclusion.

  **2. An unverifiable destination no longer refuses the sign-out.** Ending the session is what was asked for; only the redirect is unsafe. `logout()` therefore returns `redirectUri` as `undefined` instead of throwing, and the controller's existing `postLogoutUrl(...)` fallback lands the browser on this server's own authorization endpoint. That path is the normal redirect branch, so `Clear-Site-Data: "*"`, `destroySession()`, `unsetTokens()` and the session-row delete all happen exactly as they do on a validated redirect — asserted rather than assumed. This supersedes the previous entry's "an unregistered `post_logout_redirect_uri` is still refused before anything is deleted": it is now dropped, and the logout goes ahead.

  **What still refuses, before deleting anything:** a hint this server did not sign, a hint naming another issuer, an unparseable hint, a hint with no or a multi-valued audience, a `client_id` contradicting the hint's audience, and a request with neither hint nor session subject. Their tests are untouched and still green. The expired-hint acceptance via `ID_TOKEN_HINT_CLOCK_TOLERANCE` and the wrapped verification that makes a bad hint a `400` are both preserved, with tests at engine and router level. **Phase 4 ordering is preserved**: the back- and front-channel recipient lists are still collected before `deleteSessionBySubjectId`, and the engine test driving a repository that stops answering once the rows are gone still passes.

  **10 new tests; the three router-level ones were written first and confirmed failing** (all three answered `400` where they now expect `303`). In `app/http/controllers/oidc/logout.test.ts`: "honors a registered post_logout_redirect_uri with no hint and no client_id", "signs out and stays on this server when the post_logout_redirect_uri is unregistered", "ignores an unregistered post_logout_redirect_uri sent alongside a hint" — all asserting status and `Location`, since the visible symptoms were a 400 and a missing redirect. In `app/auth/oidc-provider.test.ts`: "honors a registered post_logout_redirect_uri with no hint and no client_id", "drops an unregistered post_logout_redirect_uri when no client is identified", "does not exclude the client that merely registered the redirect from the fan-out", plus "rejects invalid post_logout_redirect_uri" rewritten as "drops an unregistered post_logout_redirect_uri but still logs out". In `app/auth/repository.test.ts`: "resolves a registered logout URI to the client that registered it", "matches a logout URI exactly, never by prefix, origin or trailing slash", "returns null for an address no client registered", "answers with one stable registration when two clients share a logout URI". `bun typecheck`, `bun lint`, `bun run test`, `bun format:fix`, `bun run build` and `bunx wrangler deploy --dry-run` all pass; the dry-run still lists eleven bindings including the `POLAR_ACCESS_TOKEN` Secrets Store secret, and `dist/ssr/wrangler.json` still carries the `auth.sergiodxa.com` custom domain, the `0 0 * * *` cron and the `auth` queue consumer and producer.
...
- **`Clear-Site-Data: "*"` on the logout redirect reaches every sibling subdomain.** Its `cookies` directive is applied by browsers to the whole registrable domain, not just the origin that sent it, so a sign-out here also drops cookies held for `uptime.sergiodxa.com`, `books.sergiodxa.com` and the rest — including the edge's own `cf_clearance`. The visible symptom is a sibling subdomain answering `403` with an empty body until the browser passes a fresh challenge: an edge refusal, so nothing appears in that worker's log, and a private window loads the same URL normally. This is inherited behavior and is kept deliberately; the header only became observable once the logout redirect started completing. Diagnose it by confirming the request never reached the worker before suspecting the app.
