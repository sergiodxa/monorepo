...
1. **Every form action was failing validation unconditionally.** `@pkg/validate`'s `validate()` always flattens `FormData`/`URLSearchParams` into a plain object before handing it to the schema, but every validator in this app is built with `remix/data-schema/form-data`'s `f.object()`, which requires the raw `FormData`/`URLSearchParams` instance itself. Verified directly (reading both packages' source, then running a real `FormData` + schema through both code paths) and fixed with a local `apps/r3-uptime/app/lib/validate-form.ts` wrapper around `remix/data-schema`'s own `parseSafe()`, swapped into all 15 action files — not a change to `@pkg/validate` itself, since every other app in the monorepo pairs it with a plain-object schema that genuinely needs the conversion `@pkg/validate` does today.
