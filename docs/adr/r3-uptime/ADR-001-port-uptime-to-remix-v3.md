...
5. `RefreshFrameButton` — the manual counterpart of the auto-refresh island above: a link that reloads the `Frame` it sits inside (the dashboard's monitor table). The reload itself needs no JS — the `link` mixin's `rmx-target`/`rmx-src` attributes drive it, and the island renders them server-side so a pre-hydration click still refreshes — but a spinning icon for as long as the request is in flight has no server-rendered or CSS-only equivalent, since nothing in the DOM reflects a frame's in-flight state.
6. `PricingCalculator` — the homepage's usage-based pricing estimate. Added after Phase 9 had already shipped static pricing copy in its place (see that phase's note below): the whole feature is a total that recomputes as a visitor drags a slider and adds or removes monitors, which no server round trip or CSS-only trick expresses. Server-renders the same markup at the initial frequencies, so the panel is never blank and its sliders are still real `<input type="range">` controls pre-hydration.
...
3. Build a small server-side translator: middleware resolves the request language (order: signed-in user's `user_preferences.preferred_language` → language cookie → `Accept-Language` header → `en`) and puts a `t(key, params)` function in request context. Implementation may be i18next core (already a dependency of the OLD APP) or a typed lookup over the locale objects — pick in Phase 1 and note the choice here.
   - **Decision (Phase 1): typed lookup, not i18next core.** `app/services/translator.ts` does a dot-path lookup into the imported locale object plus a `{{param}}` regex-replace — matching the OLD APP's existing key/interpolation syntax exactly (verified against `app/locales/en.ts`) without adding an i18next runtime this server-only app doesn't otherwise need. `app/http/middleware/i18n.ts` resolves the language in the documented order and sets `ctx.t`/`ctx.language`. View-level adoption of `t()` is incremental: Phase 1's placeholder views (home, dashboard shell, auth error) use plain English copy since they have no ported strings yet; each later phase's views call `t()` against the matching keys as they're built.
     - **Reversed (2026-07-09, see the correction after the Phase 7 entry below):** now uses `@pkg/i18n` (built on i18next) instead, for consistency with other Remix v3 apps in this monorepo. `app/services/translator.ts` is deleted; `ctx.t`/`ctx.language` are replaced by `ctx.locale`/`ctx.i18next`.
...
**Live login verified (2026-07-09).** The login flow was exercised end-to-end for real against `auth.sergiodxa.com` and reached the dashboard. Getting there also surfaced and fixed four unrelated pre-existing bugs, none specific to this port: `apps/auth`'s production build was broken by upstream React Router config drift (stale deploy, fixed in commit `6bbfe56`); its worker entry never set `CloudflareContext`, so every request 500'd (`9f884df`); its `/oauth/token` endpoint only accepted client credentials via the `Authorization` header, rejecting `remix/auth`'s default body-based auth (`00effe5`); and a missing/expired authorization code mapped to a 500 instead of `invalid_grant` (`65324fa`). Separately, the NEW APP's local D1 had never had migrations applied (0 tables) — run `bun run db:local:migrate` inside `apps/r3-uptime` if you hit `DataTableAdapterError: Adapter execution failed` on a fresh checkout. The Phase 1 login gap is resolved; each phase's deeper feature-level click-through (create a monitor, trigger a check, fire an alert, etc.) still hasn't been exercised live and remains open below.
...
- [ ] Phase 2: HTTP monitors end-to-end — code complete (`app/data/monitor.ts` + `content-check.ts`, list/new/detail/edit pages, create/update/delete/play/update-ssl/content-check actions with native `<dialog>` delete confirmations, `scheduled`+`queue` worker handlers, `PingJob`, `Ping` workflow with 3-state up/degraded/down classification and dual D1+Analytics-Engine writes, `CleanJob`, `app/services/analytics.ts`, Dashboard v1 with stat cards + HTTP tab + server-rendered sparklines, `hasActiveSubscription` added to `@pkg/polar`); typecheck/lint/test/`bun format:fix` all green. **Not yet done:** a live click-through of create-monitor → trigger-check → see-result — this sandbox hits the same `COOKIE_SESSION_SECRET` secrets-store gap as Phase 1 (no local value, and creating one touches the live Cloudflare Secrets Store), so no request can get past `requireUser` locally. Whoever picks this up next should resolve the Phase 1 login gap first, then create a monitor, invoke `PingJob`/`Ping` for it, and confirm the result appears on the dashboard and detail page — then check this box.
- [ ] Phase 3: DNS, TCP, and cron-job monitors — code complete (`app/data/{dns-monitor,tcp-monitor,cron-job}.ts` with a consolidated `recordCheckResult`/`recordPing` write path per type; list/new/detail/edit pages and create/update/delete/check actions for all three, with native `<dialog>` delete confirmations on each edit page; `app/services/dns-check.ts` — DNS-over-HTTPS lookups with ok/changed/error classification; `app/services/tcp-check.ts` — real `cloudflare:sockets` `connect()` up/down/timeout checks; `CronJobMonitor` with `cron-parser`-based scheduling, `describeCronExpression`, and a genuinely public (unauthenticated, rate-limited) `POST /api/v1/cron-jobs/:cronJobId/ping` endpoint — a deliberate deviation from the OLD APP's Bearer-API-key-gated version, matching this ADR's own "public ping endpoint" phrasing and `docs/cron-job-monitoring.md`'s described flow; `CheckDnsJob`/`CheckTcpJob` (fixed-cadence sweeps of every enabled monitor, matching the OLD APP's actual — not per-monitor-interval — scheduling) and `CheckCronJobsJob` (healthy→late→missed state machine, no alert dispatch yet — that's Phase 4) plus `CleanCronJobPingsJob`; new `scheduled`/`queue` cron wiring in `bootstrap/worker.ts`; Dashboard DNS/TCP/cron-jobs tabs). Two intentional deviations from the OLD APP's current behavior, both documented in code comments: TCP checks use the real Workers `cloudflare:sockets` API instead of the OLD APP's always-`"unsupported"` feature-detection bug, and all three monitor types write a single consolidated D1 history+cached-fields row per check instead of the OLD APP's dead second write path (TCP's D1 columns are never populated in the OLD APP). typecheck/lint/test/`bun format:fix`/`wrangler deploy --dry-run` all green. **Not yet done:** live verification of any of it — this sandbox hits the same `COOKIE_SESSION_SECRET` secrets-store gap as Phases 1–2, and unlike those phases it also blocks the new _public_ ping endpoint, because `bootstrap/worker.ts`'s `fetch` handler resolves the session secret unconditionally before routing, ahead of any per-route auth check. Whoever picks this up next should resolve the Phase 1 login gap first, then exercise create/edit/delete for each monitor type, trigger a manual check, `curl -X POST` the ping URL shown on a cron-job monitor's detail page, and confirm dashboard tabs render real data — then check this box.
- [ ] Phase 4: Alerts and maintenance windows — code complete: `app/services/alerts.ts` is a single shared dispatch pipeline (`dispatchAlerts` + per-type `notifyHttpResult`/`notifyDnsResult`/`notifyTcpResult`/`notifyCronJobResult` wrappers) called from every check path — the `Ping` workflow, `CheckDnsJob`+`checkDnsMonitor`, `CheckTcpJob`+`checkTcpMonitor`, `CheckCronJobsJob`, and the cron-job ping endpoint — replacing the OLD APP's four independently-duplicated, inconsistently-behaved dispatch implementations; `app/data/alert.ts`+`alert-event.ts` (CRUD, 10-per-team limit, cooldown check, alert-history query); `app/data/maintenance-window.ts` (CRUD, end-early, recurring-aware `isActiveAt`/`isSuppressing` shared by both the list page's active/upcoming/past bucketing and alert suppression — the OLD APP's two implementations disagreed on this); full CRUD pages/actions for both alerts (four channel-config fieldsets in one form, monitor-or-team scope) and maintenance windows (monitor-or-team scope, one-time or recurring, end-early action); an alert-history page. Four deliberate improvements over the OLD APP, documented in code comments: cooldown and recovery notifications are enforced uniformly for every monitor type (the OLD APP only did this correctly for HTTP, and never supported DNS/cron-job recovery at all); webhook deliveries carry a real HMAC-SHA256 `Webhook-Signature` header (the OLD APP's docs promised this but the code never implemented it — it sent the raw secret in a header instead); maintenance-window suppression is checked before dispatch for every monitor type, not just HTTP; and monitor-scoped alerts/maintenance windows are restricted to HTTP monitors only, matching a real constraint of the reused `alerts`/`maintenance_windows` tables (no `monitor_type` column) rather than silently mismatching a DNS/TCP/cron-job id against the wrong table. Added unit tests for the recurring-pattern parser/matcher (a gap in the OLD APP). typecheck/lint/test/`bun format:fix`/`wrangler deploy --dry-run` all green. **Not yet done:** live verification — same `COOKIE_SESSION_SECRET` sandbox gap as every prior phase. Whoever picks this up next should resolve the Phase 1 login gap first, then create an alert of each channel type, trigger a monitor into a problem state, confirm delivery + `alert_events` history, create a maintenance window and confirm it suppresses alerts, and end it early — then check this box.
- [ ] Phase 5: SSL monitoring and analytics aggregation — code complete: `CheckSslJob` (daily, `0 6 * * *`) re-evaluates `calculateSslStatus` against each SSL-enabled HTTP monitor's manually-entered expiry date (Workers can't read TLS certs from `fetch()`, matching Phase 2's existing manual-entry design) and dispatches alerts via the new `notifySslResult`/`shouldAlertOnSslStatus` (`app/services/ssl-info.ts`, now unit-tested) — deliberately NOT edge-triggered, since the product wants repeated reminders at each expiry threshold, gated by per-alert cooldown instead; a dashboard SSL stat card (valid/expiring/expired counts) reads the HTTP monitors' own inline fields directly — the OLD APP's equivalent card read a standalone `ssl_monitors` table that a research pass confirmed is dead schema (defined, never populated, always renders zero), so this intentionally does not replicate that bug. `AggregateDailyStatsJob` (daily, `0 1 * * *`) writes one `monitor_daily_stats` row per monitor per day — HTTP from a new Analytics Engine query (`getHttpDailyAggregate`), DNS/TCP/cron-job from their own D1 result tables — covering all four monitor types where the OLD APP only ever aggregated HTTP and TCP (and its TCP aggregates were moot, since Phase 3 found OLD APP TCP checks always reported "unsupported"); idempotent per-day upsert via delete-then-insert. A shared `resources/views/shared/heatmap.tsx` renders each monitor type's calendar-year uptime heatmap from `monitor_daily_stats` on its own detail page (`docs/analytics.md`'s "long-term calendars", extended to all four types instead of the OLD APP's HTTP-only heatmap). `/healthcheck/analytics-engine` ported with the OLD APP's exact response contract (binding-missing → 503; read-API failure → `200 {status:"degraded"}`, not 503). typecheck/lint/test (49 passing)/`bun format:fix`/`wrangler deploy --dry-run` all green. **Not yet done:** live verification — same `COOKIE_SESSION_SECRET` sandbox gap as every prior phase. Whoever picks this up next should resolve the Phase 1 login gap first, then enable SSL monitoring on a monitor with a near-future expiry date, run `CheckSslJob` and confirm the status/alert fire, run `AggregateDailyStatsJob` and confirm a heatmap cell appears, and hit `/healthcheck/analytics-engine` — then check this box.
- [ ] Phase 6: Status pages — code complete: `app/data/status-page.ts` (CRUD, global slug uniqueness, and whole-list-resubmit curation of the four attachable monitor types — HTTP, DNS, TCP, cron-job — matching the OLD APP's own delete-and-reinsert-with-index-as-order pattern; delete also cleans up every attached-type join row, fixing the OLD APP web action's bug of leaving `status_page_cron_jobs` rows orphaned); `app/services/status-page.ts` (pure, unit-tested status derivation per monitor type plus the majority-based overall-status rule, reusing each type's already-established badge-status mapping from its own list view for internal consistency); full admin CRUD (list/new/edit, each with a checkbox fieldset per attachable type) and the public `/status/:slug` page (logo/title/description header, optional overall-status banner, one card per attached service with a badge and its 365-day heatmap via the Phase 5 shared `Heatmap` component, cron-job cards with schedule + last-ping instead of a heatmap, and an empty state). Two explicit product decisions, documented in code comments since `docs/status-pages.md` leaves them open: private pages have no access path at all via `/status/:slug` (a hard 404), matching the OLD APP's own answer rather than inventing a new one; and `status_page_ssl_monitors` is never populated — the underlying `ssl_monitors` table has no rows in production (SSL status lives inline on `monitors` instead, per Phase 5), so an SSL picker would always be empty. typecheck/lint/test (26 new tests)/`bun format:fix`/`wrangler deploy --dry-run` all green. **Live-verified via curl:** `/status/:nonexistent-slug` 404s and the authenticated admin list correctly redirects an unauthenticated request, both without crashing — confirming the routing/DB wiring works end-to-end. **Not yet done:** an authenticated click-through (create a status page, attach monitors of each type, confirm ordering/curation on save, view the public page with real data, delete) — whoever picks this up next should do that, then check this box.
- [ ] Phase 7: Team & access completion — code complete: `app/data/team.ts` gained `updateById`/`deleteById` (a full cascade — the OLD APP's delete-team action only ever cleaned up `monitors`/`alerts`/`team_domains`/`invites`/`memberships`, orphaning every DNS/TCP/cron-job monitor, status page, maintenance window, API key, and their own result/event/daily-stats history; this cascades all of it, Polar subscription cancellation handled separately by the caller), `removeMembership`/`setRole`/`createAdditional`/`listWithRoleBySubjectId`; new `app/data/{invite,team-domain,api-key,user-preferences}.ts`; `app/services/api-key.ts` (SHA-256 key generation, byte-identical `uptime_<64 hex>` format to the OLD APP's `apps/uptime/app/middleware/api-auth.ts` so existing production keys keep verifying at cutover — unit-tested); `app/jobs/{verify-domain-ownership,enqueue-pending-domains}.ts` (DNS-over-HTTPS TXT lookup, wired into the `*/10 * * * *` cron already reserved for this in Phase 0's wrangler.jsonc); the first client-side island, `resources/components/copy-button.tsx` (approved island #1 per Decision §4.5), used for the one-time API-key reveal; `@pkg/auth-sdk`'s `AuthSDK` registered in the container to resolve other members' profiles server-to-server (the session only ever carries the viewer's own identity, so team settings/account member lists need this — the OLD APP's equivalent `~/clients/auth.ts` usage is now `app/services/subjects.ts`); full pages for settings (general, members with role/remove, invite dialog, domains with verification instructions, billing link, danger zone), account (language, teams-with-leave, create-team dialog), API keys (list + one-time reveal via a session flash instead of the OLD APP's router-navigation-state trick, since every page here is a full server render; new form exposing all 20 schema-defined scopes instead of the OLD APP's stale 7-scope subset), checkout (owner → Polar-hosted checkout/portal redirect; non-owner → a message, not the OLD APP's usage-quantities view, since `@pkg/polar` has no `meters.quantities` equivalent — a deliberate scope cut, documented in `checkout.tsx`), and the public accept-invite page. `requireRole("admin")` (built in Phase 1, unused until now) gates settings/membership/invites/domains/API-keys; a route-map quirk forced splitting those actions into a new `teamAdminActions` group separate from the member-level `actions` group, since `router.map()` requires one middleware chain and every leaf of a group per call — documented in `routes/web.ts`. typecheck/lint/test (5 new tests)/`bun format:fix`/build/`wrangler deploy --dry-run` all green. **Not yet done:** any live click-through — adding the `@pkg/auth-sdk` dependency requires a dev-server restart to pick up (same class of issue as the two `apps/auth` fixes earlier), which hadn't happened yet as of this commit. Whoever picks this up next should restart the dev server, then exercise: update team name/logo, invite+accept+revoke a member, change a role, remove a member, add a domain and verify its DNS TXT record, create an API key and confirm the one-time reveal, delete an API key, change the language preference, leave a team as a plain member (and confirm owners/admins can't), create an additional team, and the owner checkout → Polar redirect — then check this box.
      **Correction (2026-07-09): DocumentLayout/createAction/i18n architectural fixes.** Three cross-cutting corrections to conventions established across Phases 1–7, all fixed together and unrelated to any single numbered phase:

1. `resources/layouts/document.tsx` was a plain function returning a render closure (`DocumentLayout()` called as a factory, then the result invoked with props), contradicting `remix/ui`'s own `Handle<Props>` component model that every other layout/view in this app already follows (e.g. `resources/layouts/app-shell.tsx`). Fixed: `DocumentLayout` is now a proper `Handle<Props>`-based component used via JSX (`<DocumentLayout title={...}>{children}</DocumentLayout>`) everywhere it's rendered (all 38 call sites across every controller).
2. Every file in `app/http/controllers/actions/` exported plain functions typed with a hand-annotated `ctx: RequestContext<{ team: string }>`, instead of using `createAction()` to derive that type from the route itself — the pattern every other single-action controller (`home.tsx`, `invite.tsx`, `status-page.tsx`) already used. Fixed: all 15 action files now export `createAction(routes.X.leaf, async (ctx) => {...})`-wrapped handlers. `bootstrap/app.tsx`'s `router.map(..., { actions: {...} })` calls still cast each value `as RequestHandler<any>` — that cast is unrelated to this fix and stays for the reason already documented there (the router's widened `Middleware[]` global-middleware array types its merged context with an opaque `any[]` entries tuple, which doesn't unify with `createAction`'s default `context = DefaultContext` `[]`-entries tuple).
3. **i18n decision reversed.** Phase 1's "Decision (Phase 1): typed lookup, not i18next core" (below) chose a custom `app/services/translator.ts` dot-path lookup specifically to avoid adding an i18next runtime. That's reversed: `app/services/translator.ts` is deleted and `app/http/middleware/i18n.ts` is now a thin config wrapper around `@pkg/i18n/middleware`'s `i18next()` factory, which does depend on and initialize a real per-request i18next instance. Reason: consistency with other Remix v3 apps in this monorepo that already standardize on `@pkg/i18n` outweighed the original runtime-avoidance rationale, once directly weighed against it. The detection order (signed-in user's `user_preferences.preferred_language` via `findLocale` → `language` cookie → `Accept-Language` header → English fallback) is unchanged; `ctx.t`/`ctx.language` are replaced by `ctx.locale`/`ctx.i18next` per `@pkg/i18n`'s contract, but no other file referenced the old properties, so this has zero other call-site impact. View-level adoption of `t()` (via `ctx.i18next`) remains incremental, as originally planned.

typecheck/lint/test/`bun format:fix`/build/`wrangler deploy --dry-run` all green.

**Correction (2026-07-09): test coverage backfill, and two critical bugs it found.** Backfilled coverage across every previously-untested layer — all 14 validators, all 15 data models (via a new shared `app/lib/test/db.ts` in-memory bun:sqlite harness applying all 33 real migrations), the 6 previously-untested services, all 9 jobs, all 7 middleware files, and all 15 form actions (68 new test files). Writing real end-to-end tests, not just inspection, surfaced two bugs that predate this session's work and would otherwise have shipped:

1. **Every form action was failing validation unconditionally.** `@pkg/validate`'s `validate()` always flattened `FormData`/`URLSearchParams` into a plain object before handing it to the schema, but every validator in this app is built with `remix/data-schema/form-data`'s `f.object()`, which requires the raw `FormData`/`URLSearchParams` instance itself. Verified directly (reading both packages' source, then running a real `FormData` + schema through both code paths). First fixed locally (a wrapper in `apps/r3-uptime`, avoiding `@pkg/validate` since every other app in the monorepo pairs it with a plain-object schema that genuinely needs the conversion). **Superseded the same day** by a real fix in `@pkg/validate` itself: `validate()` now retries with the raw `FormData`/`URLSearchParams` source specifically when the flattened-object attempt fails with the exact "Expected FormData or URLSearchParams" message — every other schema shape (Zod, core `remix/data-schema` `s.object()`) succeeds on the first attempt exactly as before, so this doesn't touch existing consumers' behavior (confirmed: `packages/validate`'s own suite, 34/34 passing including new tests for both schema kinds, plus `apps/auth`/`apps/blog`/`apps/books`/`apps/uptime`'s full suites, 388/388, all still green). The local `apps/r3-uptime/app/lib/validate-form.ts` wrapper was removed and all 15 action files reverted to `@pkg/validate`'s `validate()` directly, now that the root cause is fixed at the source.
2. **The scheduler never found any monitor due for a check.** `@pkg/data-table-d1`'s raw-SQL execution path (`db.exec()`) never returns rows for a raw `SELECT` — confirmed by reading the adapter's `execute()` directly — so `Monitor.findDue` (the once-a-minute query deciding which monitors are due) always returned `[]` in production, and `AggregateDailyStatsJob`'s DNS/TCP/cron branches would have too. Fixed directly in `packages/data-table-d1` (sniffing the raw SQL's leading keyword to decide whether to read rows), since no other consumer in the monorepo uses that path for a read; added 3 regression tests to the package's own suite. The shared test-only `app/lib/test/db.ts` adapter had an independent copy of the same bug, fixed the same way. The same gap existed independently in `packages/data-table-sqlstorage` (the production Durable Object adapter) and in two other packages' test-only mirrors of this logic (`packages/oidc-provider`, `packages/blog-engine`) — all fixed identically.
3. **Creating an alert, alert event, or API key would throw in production.** `remix/data-table`'s `c.json()` column type is just a schema tag — neither its core package nor `packages/data-table-d1`/`packages/data-table-sqlstorage` ever serialized JS objects to JSON text before binding them, so any write to `alerts.config`, `alert_events.snapshot`, or `api_keys.scopes` threw `TypeError: Binding expected string, ... or null`. This was first noticed as a test-writing inconvenience and assumed (incorrectly, without checking) to be test-infrastructure-only; a direct repro against the real adapter confirmed it also breaks production. Fixed in both `data-table-d1` and `data-table-sqlstorage` (JSON-encode `c.json()` columns on write, decode on read, using `remix/data-table`'s own column-definition introspection); `apps/r3-uptime` is the only consumer of `c.json()` in the monorepo, so this cannot regress anything else. Regression tests added to both packages' suites.

Both fixes are verified end-to-end (real script runs and/or the packages' own test suites), not just inspected. Full details and the alternatives considered are in `docs/adr/r3-uptime/AUTONOMOUS-SESSION-DECISIONS.md`. typecheck/lint/test (795 passing, 0 skipped)/`bun format:fix`/build/`wrangler deploy --dry-run` all green.

- [ ] Phase 8: API v1 — code complete: `requireApiKey(scope)` middleware (bearer extraction, SHA-256 hash lookup reusing the Phase 7 hashing helper, expiry/scope checks, `ctx.apiKey`/`ctx.apiTeam`, `last_used_at` touch); every endpoint from the URL Surface list under `app/http/controllers/api/` (monitors, dns-monitors, tcp-monitors, cron-jobs, alerts, maintenance, status-pages, invites, memberships, team, team-domains, api-keys, `/api/v1/status`, `/api/v1/backfill-daily-stats`), each registered individually in `bootstrap/app.tsx` since read/write methods on the same resource need different scopes and `router.map()` only supports one middleware chain per group call; extended the existing `routes.api` group in `routes/web.ts` with an `api.v1` subtree rather than adding the ADR-planned separate `routes/api.ts`, for consistency with the one API route (`cronJobPing`) that already lived there; `cop({ insecureBypassPatterns: ["/api/{path...}"] })` added since API v1 is bearer-authenticated, not cookie-session-based. The existing public, unauthenticated `POST /api/v1/cron-jobs/:cronJobId/ping` (Phase 3's deliberate deviation) was left untouched. Two real production bugs fixed while porting, not just replicated: the OLD APP's TCP API routes checked `tcp-monitors:read`/`write` scopes that were never actually in its own grantable scopes list, permanently unreachable — added for real; `backfill-daily-stats` was fully unauthenticated and enqueued a message type no consumer (old or new) ever handled — now gated behind `requireApiKey` and enqueues the real `aggregateDailyStats` message. typecheck/lint/test (38 passing)/`bun format:fix`/build/`wrangler deploy --dry-run` all green, plus the full merged suite (795 tests) green after merging. **Not yet done:** any live click-through against a real API key.
- [ ] Phase 9: Marketing site, docs, sitemap, polish — code complete: 35 near-identical marketing pages (12 features + 6 for/ + 7 use-cases + 10 vs/) built as 4 data-driven param routes (`/features/:slug`, `/for/:slug`, `/use-cases/:slug`, `/vs/:slug`) resolving `resources/content/marketing.ts` through 2 shared view templates, rather than 35 bespoke files; feature bullets/FAQs/comparison rows written fresh rather than porting each OLD APP page's exact prose (meta titles/descriptions carried over verbatim for SEO parity) — a deliberate low-effort choice for this `Priority: Low` phase; real homepage (replacing the Phase 1 placeholder), `/privacy`, `/terms`, `/sitemap.xml`; `/docs` + `/docs/*` serving the OLD APP's actual public docs tree (`apps/uptime/docs/**`, copied to `resources/docs/**`, distinct from this repo's own internal `apps/r3-uptime/docs/*.md` feature specs, which were never public) via a new `app/services/docs.ts`; 404 page polish. The OLD APP's interactive pricing calculator was dropped for static pricing copy (not an approved client-side island) — **since reversed**: the homepage was ported in full afterwards (split hero with the product screenshot, iconed feature/capability grids, audience pills, two-column FAQ over all 19 entries) and the calculator came with it as approved island #5, `resources/components/pricing-calculator.tsx`. i18n coverage pass explicitly skipped, per its own lowest-priority placement — all new content is English-only. typecheck/lint/test (31 passing)/`bun format:fix`/build/`wrangler deploy --dry-run` all green, plus the full merged suite (795 tests) green after merging. **Not yet done:** visual styling parity with the OLD APP (tracked separately, see the styling correction below) and any live click-through.
- [ ] Phase 10: Verification and cutover — **partially done, deliberately scoped down.** Step 1 of this phase (verify, then deploy side-by-side and browse it) is safe and reversible; steps 2-4 (moving the production route, deleting the OLD APP) are not something to do without the app's owner present, so only the safe, non-destructive parts were done: re-ran the full verification suite (typecheck/lint/test 795 passing/format/build/`wrangler deploy --dry-run`, all green) on the fully-merged app; cross-checked every route in `routes/web.ts` against the ADR's own "URL Surface (parity required)" list by direct inspection (not by trusting any prior phase's self-report) — full coverage confirmed, with a few pages the OLD APP lists separately (`/app/:team/domains/new`, `/app/:team/settings/invite`, `/app/:team/account/create-team`) implemented as inline dialogs on their parent page instead of standalone routes, a Phase 7 decision, not new; wrote `apps/r3-uptime/README.md`'s "Cutover" section as the condensed runbook for whoever executes steps 1-4 for real. **Not done, and should not be done autonomously:** the actual side-by-side deploy, the production route move, the week-long soak, and deleting the OLD APP — these need the app's owner's direct involvement and sign-off. Do not check this box until all four steps in the README's runbook are actually complete.

**Correction (2026-07-09): visual styling actually matched to the OLD APP.** `resources/styles.ts` had shipped with a generic gray/blue palette in plain hex, unrelated to the OLD APP's real design — confirmed by the user directly ("right now they don't match at all") and by inspection (`apps/uptime/app/assets/styles.css`'s `@theme` block uses OKLCH throughout, a green-hued neutral scale and green primary brand color, amber/red/teal-green semantic colors, and `Mona Sans`). Fixed: every one of `resources/styles.ts`'s ~90 mixins now uses the OLD APP's real OKLCH token values (neutral hue 145, primary/green 142, warning/amber 85, danger/red 25, success/teal-green 155) and font stacks, cross-checked against how the OLD APP's actual components use them (hero gradients, header/footer link colors, status-badge colors, CTA gradients); a few view/component files with hardcoded hex colors bypassing the shared tokens were fixed too (`resources/components/copy-button.tsx`, plus new `documentBody`/`marketingFont` mixins so the body background and the marketing-vs-docs font split match the OLD APP's `root.tsx`/`_landing.tsx` split). One real bug found only through visual verification in a real browser (not by reading code): a marketing card component rendered as an unstyled `<a>` link — default browser link-blue, no underline reset — fixed with an explicit `color`/`text-decoration` reset. Not translated: the OLD APP self-hosts the `Mona Sans` font via `@font-face` + a `.woff2` file; this app's `css()` mixin system has no global stylesheet/asset pipeline to host that, so the font stack gracefully falls back to the system UI sans-serif instead — flagged as infrastructure work beyond a styling pass, not attempted. typecheck/lint/test (795 passing)/`bun format:fix`/build/`wrangler deploy --dry-run` all green; visually spot-checked in a real browser (green-primary, dark-mode homepage confirmed via screenshot during this session).
