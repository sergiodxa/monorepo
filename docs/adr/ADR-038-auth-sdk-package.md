...
A configuration fault caught at construction is a plain `Error` for the same reason: the provider has not been contacted, so every code available would name a failure that has not happened. The message names the option that resolves it. A caller-supplied reserved parameter is the exception — `reserved_parameter` describes the caller rather than the provider, so it keeps its code.
