...
That applies to signing secrets as much as to API tokens, because the constraint is where a secret lives rather than which secret it is. An unreadable signing secret makes a delivery unproven, because a webhook endpoint that answers 5xx is how a platform ends up disabling it.
