...
a comment in the migration and an assertion in the test suite: *a completed check writes
exactly two rows — the result and the alarm.*
...
- **Cadence.** Property-style: for a monitor with interval *i*, delayed by *d*, the next
...
- The cost ledger (ADR-007) must be re-implemented inside the objects. It gets *better*
