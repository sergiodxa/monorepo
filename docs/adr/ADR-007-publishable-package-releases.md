...
| npm refuses to run anywhere inside this repository: the root `devEngines` entry names Bun with `onFail: "download"`, which npm reports as `EBADDEVENGINES` for `view`, `publish`, `login` and `whoami` alike | Every npm invocation runs with `cwd` outside the repository, in the staging directory under the OS temp dir; `npm login` runs from outside the repository too                                                                                                                          |
...
1. Checks `npm whoami` for an operator session; `npm login` is run beforehand from a directory outside the repository.
...
3. From a directory outside the repository, `npm login`. Then `bun run release:bootstrap @sdxc/<name>` publishes `0.0.0-pre.1` and prints the trusted-publisher settings.
...
- `npm view`, `npm publish`, `npm login` and `npm whoami` all fail inside the repository with `EBADDEVENGINES`, because the root `devEngines` names Bun with `onFail: "download"`. The scripts run npm with `cwd` in the staging directory under the OS temp dir; an operator runs `npm login` and any manual `npm view @sdxc/<name>` from a directory such as the home directory.
