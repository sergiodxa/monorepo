...
- `apps/uptime/app/jobs/check-dns.ts`
- `apps/uptime/app/jobs/check-tcp.ts`
- `apps/uptime/app/jobs/check-cron-jobs.ts`
- `apps/uptime/app/jobs/check-ssl.ts`
...
CLOUDFLARE_ANALYTICS_API_TOKEN=<configured>
```

The API token needs the `Analytics: Read` permission.
...
- [x] Environment variables configured (`CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_ANALYTICS_API_TOKEN`)
...
**Phase 1: Schema Changes** - Update `apps/uptime/db/schema.ts` with:

1. `sslMonitors` table (start empty; no migration of existing SSL columns)
2. `monitorDailyStats` table
3. `statusPageDnsMonitors` table
4. `statusPageTcpMonitors` table
5. `statusPageSslMonitors` table
6. Add nullable `monitorType`, `monitorName`, `snapshot` columns to `alertEvents`
7. Add all relations
