...
function TextField({ className }: Props) {
	return (
		<div className={cn("v-stack gap-1", className?.root)}>
			...
			{error && <p className={cn("text-sm text-failure-600", className?.error)}>{error}</p>}
		</div>
	);
}
...
```tsx
type Props = {
	className?: ClassNameRecord<"overlay" | "container" | "header" | "body" | "footer">;
};
	...
	return (
		<div className={cn("fixed inset-0 bg-neutral-900/50", className?.overlay)}>
			<div className={cn("bg-white rounded-xl", className?.container)}>
				<header className={cn("p-4 border-b", className?.header)}>{title}</header>
				<div className={cn("p-4", className?.body)}>{children}</div>
				<footer className={cn("p-4 border-t", className?.footer)}>{actions}</footer>
			</div>
		</div>
	);
}
...
function ListItem({ className, icon, children, onAction }: Props) {
	return (
		<div className={cn("h-stack items-center gap-3 p-3", className?.root)}>
			...
			{onAction && (
				<button className={cn("shrink-0", className?.action)} onClick={onAction}>
					Action
				</button>
			)}
		</div>
