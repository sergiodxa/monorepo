...
The directory under `packages/` and the segment after `@pkg/` are the same kebab-case
...
- **The import path is the only name most readers ever see.** `@pkg/data-table-d1`
...
```text
# Bad
packages/dataTableD1/     -> "@pkg/data-table-d1"
packages/session_kv/      -> "@pkg/session-storage-kv"

# Good
packages/data-table-d1/       package.json -> "@pkg/data-table-d1"
packages/session-storage-kv/  package.json -> "@pkg/session-storage-kv"
README.md                     -> "# @pkg/data-table-d1"
```

Name the package for what it is, not for who uses it: `@pkg/hostname`, not
`@pkg/blog-saas-hostnames`. A name that carries an app in it is a name that stops being
true the second app adopts it.

## Rules

1. Directory name and the `@pkg/` segment are the same kebab-case string
2. README H1 is the full scoped name, `# @pkg/<name>`
