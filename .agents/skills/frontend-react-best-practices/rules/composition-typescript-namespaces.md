...
export function Button({
	...
	return (
		<button className={getButtonClasses(variant, size)} onClick={onClick} disabled={disabled}>
			{children}
		</button>
	);
}
...
import { Button, ButtonProps, ButtonVariant, ButtonSize } from "~/components/button";
