/**
 * Covers key generation, the JSON round-trip a key survives in storage, the JWKS
 * document the public half publishes as, and resolving a key set — local or fetched
 * — into something a token verifies against, including a rotation that pages through
 * several stored keys and keeps each one resolvable by its own `kid`.
 *
...
/** Pinned to one algorithm, guarding verification against a key that claims another. */
...
class MemoryKeyStorage implements KeyStorage {
	 ...
	 *
	 * An object store may return fewer entries than requested, so callers walk the
	 * cursor across as many pages as it takes to cover the requested limit.
	 *
