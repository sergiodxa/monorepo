...
```tsx
import { HeartIcon } from "@pkg/lucide-remix";
...
```tsx
import { CircleAlertIcon } from "@pkg/lucide-remix";
...
```tsx
import { TrashIcon } from "@pkg/lucide-remix";
...
```tsx
import { Icon } from "@pkg/lucide-remix";
...
```typescript
import { createLucideIcon } from "@pkg/lucide-remix";
...
```bash
bun install
bun run --cwd packages/lucide-remix generate
bun format:fix
```
...
1. **Import only what you use** - Each icon is its own module (`sideEffects: false`), so bundlers tree-shake unused icons as long as you import by name from `@pkg/lucide-remix` rather than a wildcard.
