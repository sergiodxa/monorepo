 ...
 * of the shared composite `boxShadow` declaration. Because it claims only that
 * one slot, an elevation shadow and a `u.ringShadow()` on the same element
 * render as two stacked layers instead of the later utility erasing the
 * earlier one.
 *
 * @example u.shadow("lg")
