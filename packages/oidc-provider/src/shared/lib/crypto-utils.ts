 ...
 * @example
 * let sig = await hmacSign("user:123", process.env.SESSION_SECRET);
 */
 ...
 * @example
 * base64UrlEncode("a+b/c"); // "YStiL2M" (URL-safe, no padding)
 */
 ...
 * @example
 * if (!constantTimeCompare(providedSignature, expectedSignature)) return false;
 */
