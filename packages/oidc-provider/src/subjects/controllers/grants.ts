 ...
 */
export const index: Action<typeof routes.api.subjects.grants.index> = createAction(
	...
	async (ctx) => {
		...
		let grants = await Grant.listBySubject(ctx.db, id);
 ...
 */
export const destroy: Action<typeof routes.api.subjects.grants.destroy> = createAction(
	...
	async (ctx) => {
		...
		let grant = await Grant.show(ctx.db, grantId);
		if (!grant) {
		...
		try {
			await Grant.destroy(ctx.db, grantId);
