 ...
 */
const verifyEmailController: Action<typeof routes.verifyEmail> = createAction(
	routes.verifyEmail,
	async (ctx) => {
		let { request, log } = ctx;
		...
		try {
			let { subjectId } = await EmailVerificationToken.consume(ctx.db, token);
			await Subject.verifyEmail(ctx.db, subjectId);
