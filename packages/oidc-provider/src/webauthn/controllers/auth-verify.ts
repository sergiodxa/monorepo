 ...
 */
const authVerifyController: Action<typeof routes.webauthn.auth.verify> = createAction(
	...
	async (ctx) => {
		...
		let subject = await Subject.show(ctx.db, challenge.subject_id);
		if (!subject) {
		...
		await Passkey.updateCounter(ctx.db, passkey.id, verification.authenticationInfo.newCounter);
...
export default authVerifyController;
