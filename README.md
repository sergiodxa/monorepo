...
| `@cloudflare/workers-types`       | `blog-engine`, `cache`, `cloudflare-mocks`, `data-table-d1`, `data-table-sqlstorage`, `jobs`, `kv-cache`, `logger`, `oidc-provider` | Workerd runtime types; apps generate theirs with `wrangler types`.    |
