...
export default async function loginWithCredential(input: Input) {
	...
	if (subject) {
		...
		if (!credential) {
			...
			return failure(new OIDCProvider.MissingValidationError("Verify your email address."));
		}
		...
		if (!passwordValid) {
			...
			return failure(new OIDCProvider.AccessDeniedError("Invalid email or password."));
		}
	} else {
