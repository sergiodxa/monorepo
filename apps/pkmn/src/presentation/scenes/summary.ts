...
export class SummaryScene implements Scene {
	...
	render(game: GameClient, ctx: CanvasRenderingContext2D) {
		...
		drawText(ctx, `Species: ${creature.speciesId}`, 14, 26, { color: "#404040" });
		drawText(ctx, `HP ${creature.currentHP}/${creature.maxHP}`, 14, 40, { color: "#404040" });
		drawText(ctx, `Status: ${creature.status ?? "OK"}`, 140, 40, { color: "#404040" });

		Window.frame(ctx, 6, 64, 228, 88);
		drawText(ctx, "MOVES", 14, 70, { color: "#202020" });
		creature.moves.forEach((move, index) => {
			let y = 86 + index * 15;
			drawText(ctx, move.id ?? "-", 20, y);
			drawText(ctx, `PP ${move.pp}`, 220, y, { align: "right", color: "#606060" });
		});
	}
}
