 ...
 * pattern. The component picks one species from the real roster, seeds a
 * {@link SpeciesEditor} with it, and drives every control through it: dex
 * number/size, base stats, primary/secondary types, growth rate, catch rate and
 * base experience, gender ratio, EV yield, the level-up learnset (add/remove/sort
 * level → move rows), evolutions (method + target + level/item), and the sprite
 * association (an atlas region or a manifest image). There are no framework hooks:
 ...
 * validates it, replaces that one entry in `src/content/species.json` behind the
 * shared path-safety guard, and reports where the file landed inline.
 *
...
/**
 * Species-content authoring tool. Loads a picked species into a
 * {@link SpeciesEditor}, renders controls for every editable field around it, and
 * exports the edited record back into `src/content/species.json` on demand.
 *
...
export function SpeciesTool(handle: Handle<Record<string, never>>) {
	...
	/** Renders the species picker plus dex number, size, catch rate, and base exp. */
	...
	/** Renders the type, growth-rate, and egg-group controls. */
	...
	function renderStats() {
		return (
			...
			>
				<legend mix={css({ fontSize: "1rem", padding: 0 })}>Base stats</legend>
	...
	/** Renders the gender-ratio controls (genderless toggle + male/female %). */
	...
	/** Renders the EV-yield number inputs (0 removes a stat's yield). */
	function renderEvYield() {
		let yields = species.evYield ?? {};
		return (
			<fieldset
				mix={css({ border: "none", margin: 0, padding: 0, display: "grid", gap: "0.5rem" })}
			>
				<legend mix={css({ fontSize: "1rem", padding: 0 })}>EV yield</legend>
				<div mix={css({ display: "flex", flexWrap: "wrap", gap: "0.75rem" })}>
					{STAT_VALUES.map((stat) => (
						<label key={stat} mix={LABEL}>
							{stat}
	...
	/** Renders the learnset builder: rows plus add/sort controls. */
	...
	/** Renders the evolutions builder: rows plus the add control. */
	function renderEvolutions() {
		return (
			<div mix={css({ display: "grid", gap: "0.5rem" })}>
				<div mix={css({ display: "flex", gap: "0.75rem", alignItems: "center" })}>
					<h3 mix={css({ margin: 0, fontSize: "1rem" })}>Evolutions ({editor.evolutionCount})</h3>
					<button
						type="button"
						mix={[
							css(CONTROL_BUTTON),
	...
	/** Renders the sprite association controls (none / atlas region / image). */
	function renderSprite() {
		...
		return (
			<div mix={css({ display: "grid", gap: "0.5rem" })}>
				<h3 mix={css({ margin: 0, fontSize: "1rem" })}>Sprite</h3>
				<div
		...
		<section mix={css({ display: "grid", gap: "1.5rem", justifyItems: "stretch" })}>
			<header mix={css({ display: "grid", gap: "0.25rem" })}>
				...
				<p mix={css({ margin: 0, color: "#9ca3af", fontSize: "0.85rem" })}>
					Pick a species, edit its stats, types, learnset, evolutions, and sprite, then export it
					back into <code>src/content/species.json</code>.
				</p>
			</header>
