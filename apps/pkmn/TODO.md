...
## Data-driven maps (format + loader/renderer) follow-ups

- [ ] Implement the event runtime: the map JSON `events` schema is complete and validated (`src/presentation/render/map-schema.ts`), but the overworld still runs the hand-built `createSampleNpcs` NPCs (`src/presentation/overworld/map-loader.ts`) and does not yet act on loaded `events` (moving NPCs, `wild`/legendary fixed encounters, `touch`/`autorun` triggers, `interactionMode`, `flag`/`once` gating). Build the runtime that spawns actors and runs `interaction.script` from a map's `events`.
- [ ] Build the map EDITOR against this format: `MapDataSchema` is the contract (packed layer cells via `packTileRef`, multi-tileset, three layers, events). The dev editor phase should author + export map JSON to `src/content/maps/` and register it in the manifest.
