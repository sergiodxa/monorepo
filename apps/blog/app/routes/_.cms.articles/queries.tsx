...
export const MarkdownSchema = z
	...
	.transform((content) => {
		if (content.startsWith("# ")) {
			let [title, ...body] = z.string().array().min(1).parse(content.split("\n"));
		...
		let [title, ...body] = z.string().array().min(1).parse(content.trim().split("\n"));
		let plain = body.join("\n").trimStart();
...
export async function moveToTutorial(id: UUID) {
	...
	await bindings.kv.redirects.put(slugMeta.value, JSON.stringify({ from, to }), {
		metadata: { from, to },
	});
}
