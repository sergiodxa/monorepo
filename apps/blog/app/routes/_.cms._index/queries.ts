...
export async function queryStats() {
	...
	let [articles, likes, tutorials, glossary] = await Promise.all(
		["article" as const, "like" as const, "tutorial" as const, "glossary" as const].map(
			async (type) => {
				let results = await db.select({ value: count() }).from(posts).where(eq(posts.type, type));
...
export async function queryLastDaySearch() {
	...
	return {
		...
		tutorials: [...tutorials, ...feedTutorials]
			.map((initialKey) => {
				let key = initialKey.replace("tutorials:search:", "").replace("feed:", "");
...
async function listCache(prefix?: string, limit = 1000) {
	...
	return list.keys.map((key: { name: string }) => key.name);
}
