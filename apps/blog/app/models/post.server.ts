...
export class Post<Meta extends BaseMeta> {
	...
	static async create<Meta extends BaseMeta>(
		...
		return await measure("Post.create#show", () => Post.show<Meta>({ db }, type, id));
	}
	...
	static async update<Meta extends BaseMeta>(
		...
		await measure("Post.update#meta", () =>
			db.delete(schema.postMeta).where(eq(schema.postMeta.postId, id)).execute(),
		);
		...
		return await measure("Post.update#show", () => Post.show<Meta>({ db }, type, id));
	}
}

async function createPostMeta(db: Database, id: UUID, key: string, value: unknown): Promise<void> {
	if (!value) return;

	if (typeof value === "string") {
		return void (await db.insert(schema.postMeta).values({ postId: id, key, value }).execute());
	}
...
function reduceMeta<Meta extends BaseMeta>(meta: schema.SelectPostMeta[]): Meta {
