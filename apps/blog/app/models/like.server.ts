...
export class Like extends Post<LikeMeta> {
	...
	static override async list({ db }: Services) {
		let posts = await measure("Like.list", () => Post.list<LikeMeta>({ db }, "like"));
	...
	static override async show({ db }: Services, id: schema.SelectPost["id"]) {
		let post = await measure("Like.show", () => Post.show<LikeMeta>({ db }, "like", id));
		return new Like({ db }, post);
	}

	static override async create({ db }: Services, { title, url, ...input }: InsertLike) {
	...
	static override async update(services: Services, id: schema.SelectPost["id"], input: InsertLike) {
