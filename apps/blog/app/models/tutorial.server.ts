...
export class Tutorial extends Post<TutorialMeta> {
	...
	static async search(services: Services, searchQuery = "") {
		...
		for (let techInQuery of techsInQuery) {
			if (techInQuery.version) {
				query = query.replace(`tech:${techInQuery.name}@${techInQuery.version}`, "");
			} else {
			...
			tutorials = tutorials.filter((item) => {
				for (let tagInTutorial of item.tags) {
					...
					if (techInQuery.name.includes("*")) {
						if (!techInTutorial.name.includes(techInQuery.name.replace("*", ""))) {
							continue;
						}
	...
	static override async show(services: Services, slug: schema.SelectPostMeta["value"]) {
	...
	private static getPackageNameAndVersion(tag: string) {
		...
		if (!tag.startsWith("@")) {
			...
			if (result.error) {
				logger.info("parse-tag-failed", { tag, issues: result.error.issues });
				return { name: "", version: "" };
			}
		...
		if (result.error) {
			logger.info("parse-scoped-tag-failed", { tag, issues: result.error.issues });
			return { name: "", version: "" };
		}
