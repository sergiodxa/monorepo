...
This document defines app-specific rules for `apps/blog`.
...
- MUST read the database in HTTP handlers from `ctx.db`, published by the global `database()` middleware in `app/http/middleware/database.ts`.
