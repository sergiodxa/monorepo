			...
			<main mix={[css({ display: "grid", gap: "0.9rem", margin: "0 auto" })]}>
				...
				{related.length > 0 && (
					<section mix={[css({ display: "grid", gap: "1rem" })]}>
						<h2
							mix={[css({ margin: 0, color: "var(--ui-neutral-fg-emphasis)", fontSize: "1.5rem" })]}
						>
							Related tutorials
						</h2>
						<div
				...
				>
					...
					<dd mix={[css({ margin: 0 })]}>{typePath}</dd>
					<dt mix={[css({ fontWeight: 700, color: "var(--ui-neutral-fg)" })]}>Slug</dt>
					<dd mix={[css({ margin: 0 })]}>{slug}</dd>
					...
					{publishedAt && (
						<>
							...
							<dd mix={[css({ margin: 0 })]}>{formatDate(publishedAt)}</dd>
						</>
					)}
				</dl>
			</main>
