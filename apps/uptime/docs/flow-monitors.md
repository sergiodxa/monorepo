...
- Only HTTP requests are counted against the request cap. `url` and `jwt` calls are free, which is right while HTTP is the only cost, and is narrower than a cap on tool calls in general.
