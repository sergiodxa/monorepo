...
- `html` is free by the same rule but is the one free call with real cost: it parses the source string it is handed, and a response body has no size cap, so a large page bounds a run's CPU rather than its request count. Repeated reads of one response are cheap — the parse is cached per source string, so twenty assertions over one body parse it once — which leaves the body's own size as the cost that remains. A cap on what `http` will read is what would bound that deliberately; the isolate's CPU limit is what bounds it today.
