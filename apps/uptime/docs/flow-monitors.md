...
- `html` is free by the same rule but is the one free call with real cost: it parses the source string it is handed, and a response body has no size cap, so a large page bounds a run's CPU rather than its request count. Each call re-parses from scratch, so a source asserting twenty times over one response parses that response twenty times. The isolate's own CPU limit is what stops it today; a body cap on `http`, or a parse cached per source string, is what would stop it deliberately.
