...
Exactly seven namespaces are registered: `http`, `url`, `jwt`, `sample`, `html`, `str` and `spec`. Files, shells, databases and environment variables are **refused, not merely absent** — a spec reaching for a file is answered with the language's own permission error naming the path it tried, so a habit earns a readable sentence instead of a crash. There is no browser: `html` parses a response body in process and addresses it by role and accessible name, which covers a server-rendered page and stops short of anything a layout answers.

`expect` compares two values for equality, asserts that one value is true, or reads an observable. There is no contains, no regex, no comparison and no range — substring matching lives in the `html` readers' own `containing` clause. The language has no branching and no arithmetic; `str.format` is the whole of string building, and `spec.nonce` is the value a flow composes into an identifier that has to be new each run. `sample` covers the value a flow needs fresh each run, one tool per kind returning a record — a person, a place, an identifier, a date, a file — which is what a sign-up flow drives; a flow that signs in reads its credential from a fixed account written into the source.
...
- A run reads at most `1,048,576` bytes of any one response body. Parsing markup costs about 25 times the source, against a `128` MB isolate, so a single unbounded response would exhaust the memory the run and every monitor sharing the isolate need.
...
- All three run caps report `error`, on the reading that a spec asking for more time, more requests or a larger response than the product allows is a monitor to fix rather than an outage. The size refusal carries its own error type, so recognising it reads a field rather than the wording of a message this app did not write.
- The response cap is enforced as the body arrives, not after it is in memory: a `content-length` past the cap refuses before a byte is read, and a response that declares none has its bytes counted as they stream, cancelling the moment the count passes the cap. Measuring a buffered body would already have spent the memory the cap exists to protect. It bounds one body at a time, so twenty near-cap responses in one run are allowed and the request cap is what bounds their total.
...
- Only HTTP requests are counted against the request cap. `url`, `jwt`, `sample`, `str` and `spec` calls are free, which is right while HTTP is the only cost, and is narrower than a cap on tool calls in general.
- `html` is free by the same rule but is the one free call with real cost: it parses the source string it is handed, so a large page bounds a run's CPU and memory rather than its request count. Repeated reads of one response are cheap — the parse is cached per source string, so twenty assertions over one body parse it once — which leaves the body's own size as the cost that remains, and the response cap is what bounds it: `http` refuses a body past `1,048,576` bytes, so nothing that large ever reaches a parse.
...
- A flow whose steps are requests is not a substitute for watching a browser. `html` reads the markup a server sent, so a missing heading or a vanished button is visible; a failure that only appears once scripts have run and the page is laid out is not.
