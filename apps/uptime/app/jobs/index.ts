 ...
 * any job's dependencies. Each key is the name a message is addressed by on the wire —
 * renaming one renames a message the previous deploy may still have in flight.
 *
...
export default jobs({
	...
	checkCronJobs: job({
		cron: "* * * * *",
		meta: { monitorId: "70a5dba9-8447-4cc0-a5f6-d0e41dc6b9e5" } satisfies Monitored,
	}),
	checkTcp: job({
		cron: "* * * * *",
		meta: { monitorId: "94276ec1-18f9-4dde-8a09-c5a00df29454" } satisfies Monitored,
	}),
	checkDns: job({
		cron: "* * * * *",
		meta: { monitorId: "3a620acd-43f9-4f48-9a32-b9a87698e44e" } satisfies Monitored,
	}),
	checkFlows: job({ cron: "* * * * *" }),
	enqueuePendingDomains: job({
		cron: "*/10 * * * *",
		meta: { monitorId: "9a2e4fe3-f5fe-4365-8b8f-2f2d90d6101c" } satisfies Monitored,
	}),
	...
	clean: job({
		cron: "0 0 * * *",
		meta: { monitorId: "80294988-476e-4e99-9f5c-abfeb369316a" } satisfies Monitored,
	}),
	cleanCronJobPings: job({
		cron: "0 0 * * *",
		meta: { monitorId: "31db20cb-8736-44fa-9ac7-448d2200befd" } satisfies Monitored,
	}),
	aggregateDailyStats: job({
		cron: "0 1 * * *",
		meta: { monitorId: "3f5a0689-1ced-4fcc-826d-3c1dc3c2795e" } satisfies Monitored,
	}),
	/**
	...
	reconcileSubscriptions: job({
		cron: "0 2 * * *",
		meta: { monitorId: "2df6d6d9-54e9-4a84-954a-e1d357421459" } satisfies Monitored,
	}),
	reportCosts: job({
		cron: "0 3 * * *",
		meta: { monitorId: "ddf291cc-5fd5-4ab7-b016-dea824399990" } satisfies Monitored,
	}),
	/**
	...
	sendTeamDailyDigests: job({
		cron: "0 8 * * *",
		meta: { monitorId: "03acb710-cd5b-4c8a-8242-c2a2a9dae201" } satisfies Monitored,
	}),
