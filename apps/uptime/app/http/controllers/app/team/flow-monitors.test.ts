...
async function send(
	...
	router.map(routes.app.team.flowMonitors.index, {
		middleware: [seedTeam(team, membership), i18n, renderWith(createHtmlRenderer) as Middleware],
		handler,
	});

	let request = new Request(
		new URL(routes.app.team.flowMonitors.index.href({ team: team.slug }), "https://uptime.test"),
	);
...
describe("flowMonitors", () => {
	test("renders the empty state when the team has no flow monitors", async () => {
		...
		expect(body).toContain("Flow Monitors");
		expect(body).toContain("No flow monitors yet");
	});

	test("lists a monitor with its interval and its not-yet-checked status", async () => {
		let { db, team, membership } = await createFixture();
		await seedMonitor(db, team.id, { name: "Checkout", interval_seconds: 900 });
	...
	test("a failing monitor reads as failing and a disabled one says so", async () => {
		let { db, team, membership } = await createFixture();
		await seedMonitor(db, team.id, { name: "Broken", last_status: "down" });
		await seedMonitor(db, team.id, { name: "Paused", is_enabled: false });
	...
	test("another team's monitors are not listed", async () => {
		let { db, team, membership } = await createFixture();
		...
		await seedMonitor(db, other.id, { name: "Not Yours" });
