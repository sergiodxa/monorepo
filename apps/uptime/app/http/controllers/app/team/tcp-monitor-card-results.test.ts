/**
 * Tests for the TCP monitor results fragment controller — the stat cards derived from
...
let { handler } = (await import("./tcp-monitor-card-results")).default as {
...
async function send(
	...
	router.map(routes.app.team.tcpMonitors.cards.results, {
	...
	let request = new Request(
		new URL(
			routes.app.team.tcpMonitors.cards.results.href({ team: team.slug, monitorId }),
			"https://uptime.test",
		),
	);
...
describe("tcp-monitor-card-results", () => {
	test("renders the derived stat cards and the empty result table", async () => {
		...
		expect(body).toContain("No check results yet. Results will appear after the first check runs.");
	});
