...
/**
 * Matches `bootstrap/app.tsx`'s `renderWith(createHtmlRenderer)` wiring; a
 * single-request page test only needs `resolveFrame` to return an empty
 * string.
 */
...
describe("GET /status/:slug", () => {
	...
	/**
	 * The public page publishes only the operator-chosen label — the monitor's
	 * internal name stays private to the team.
	 */
	...
	/**
	 * A cleared label field holds whitespace, so the display must still fall
	 * back to the monitor's own name.
	 */
	...
	/**
	 * A status page is world-readable, so it publishes only domain-wide
	 * coverage — the domain, its records, and the values they resolve to
	 * stay in the owner's infrastructure map.
	 */
	...
	/**
	 * `getTeamHttpSummaries` queries Analytics Engine for every team regardless
	 * of its monitors, so the SQL stub is required even here. The canonical
	 * link always resolves to the product's own origin.
	 */
	...
	/**
	 * `max-age=60` mirrors the KV TTL behind the page, keeping responses as
	...
	/**
	 * The rendered timestamp is rounded to the cache window, so a second
	 * render inside that window is byte-identical and the viewer's tag
	 * still matches.
	 */
