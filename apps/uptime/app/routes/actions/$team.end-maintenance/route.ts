...
export async function action({ request, context }: Route.ActionArgs) {
	...
	if (isFailure(result)) {
		logger().info("action.end-maintenance.validation-failed", {
			issues: result.error.issues,
		});
		return badRequest({ message: t("actions.endMaintenance.errors.generic") });
	}
	...
	if (!maintenance) {
		logger().info("action.end-maintenance.not-found", {
			maintenanceId: result.data.maintenanceId,
		});
		return notFound({ message: t("actions.endMaintenance.errors.notFound") });
	}

	if (maintenance.teamId !== team().id) {
		logger().info("action.end-maintenance.forbidden", {
			maintenanceId: result.data.maintenanceId,
			maintenanceTeamId: maintenance.teamId,
			requestTeamId: team().id,
		});
		return forbidden({
			message: t("actions.endMaintenance.errors.forbidden"),
		});
	}

	// Set endedEarlyAt to now to mark maintenance as ended early
	await db()
		.update(schema.maintenanceWindows)
		.set({ endedEarlyAt: new Date() })
		.where(eq(schema.maintenanceWindows.id, maintenance.id));

	logger().info("action.end-maintenance.success", {
		teamId: team().id,
		maintenanceId: maintenance.id,
	});

	return ok({
		message: t("actions.endMaintenance.success", { name: maintenance.name }),
	});
}
