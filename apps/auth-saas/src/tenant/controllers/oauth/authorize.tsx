...
// Script generator for client entry to use
export function generateWebAuthnScript(props: WebAuthnScriptProps): string {
	if (props.type === "authenticate") {
		return `
			(async function() {
				const data = document.getElementById('webauthn-data').dataset;
				const challengeId = data.challengeId;
				const challenge = data.challenge;
				const rpId = data.rpId;
				const allowCredentials = JSON.parse(data.allowCredentials || '[]');
				
				const container = document.getElementById('webauthn-auth');
				container.innerHTML = '<p>Waiting for passkey...</p>';
				...
				try {
					...
					const res = await fetch('/webauthn/auth/verify', {
						method: 'POST',
					...
					if (result.redirect) {
						...
						container.innerHTML = '<p style="color: #DC2626;">Error: ' + result.error + '</p><button onclick="location.reload()">Try Again</button>';
					}
				} catch (err) {
					container.innerHTML = '<p style="color: #DC2626;">Error: ' + err.message + '</p><button onclick="location.reload()">Try Again</button>';
				}
			})();
		`;
	} else {
		return `
			(async function() {
				...
				const challenge = data.challenge;
				const rpId = data.rpId;
				const rpName = data.rpName;
				const email = data.email;
				
				const container = document.getElementById('webauthn-register');
				container.innerHTML = '<p>Creating passkey...</p>';
