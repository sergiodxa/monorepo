...
export function layout(options: LayoutOptions): SafeHtml {
	...
	if (backLink && backText) {
		breadcrumb = html`<a href="${backLink}" class="text-gray-600 hover:text-gray-900"
			>&larr; ${backText}</a
		>`;
	} else if (tenant) {
		breadcrumb = html`<a
			href="${routes.dashboard.tenants.show.href({ id: tenant.id })}"
			...
			<div class="bg-yellow-50 border-b border-yellow-200">
				<div class="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
					<p class="text-yellow-800 text-sm">
						<strong>Payment past due:</strong> Your subscription payment has failed. Please update
						your payment method to avoid service interruption.
					</p>
					<a
						href="${subscriptionWarning.billingUrl}"
	...
	if (tenant) {
		tenantBreadcrumb = html`<span class="text-gray-400">/</span
