...
export default action<"GET", "/dashboard">(async ({ db, platformSession, logger }) => {
	...
	let tenants = await Tenant.listByOwner(db, platformSession.subjectId);
	...
	if (tenants.length === 0) {
		return new Response(null, {
			status: 302,
			headers: { Location: "/dashboard/tenants/new" },
		});
	}
		...
		<body class="bg-gray-50 min-h-screen">
			...
			<main class="max-w-6xl mx-auto px-4 py-8">
				<div class="flex justify-between items-center mb-6">
					...
					<a href="/dashboard/tenants/new" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">
						New Tenant
					</a>
				</div>
