/**
 * Behavioural tests for the shared crypto helpers: `hmacSign` produces a stable,
 * base64url-encoded HMAC-SHA256 signature that varies with the input and secret;
 * `base64UrlEncode`/`base64UrlDecode` round-trip strings and byte arrays without
 * `+`, `/`, or `=`; and `constantTimeCompare` matches only identical strings.
 *
