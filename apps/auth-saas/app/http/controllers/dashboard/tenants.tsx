...
export default {
	show: createAction(routes.dashboard.tenants.show, async (ctx) => {
		let { db, params, platformSession, log } = ctx;
		...
		return ctx.render(
			<Document title={tenant.name} backLink={routes.dashboard.index.href()} backText="Dashboard">
				...
				<div mix={[s.cardGrid]}>
					...
					<a mix={[s.linkCard]} href={routes.dashboard.tenants.users.index.href({ tenantId: id })}>
	...
	new: createAction(routes.dashboard.tenants.new, (ctx) => {
		return ctx.render(
			<Document
				title="New Tenant"
	 ...
	 */
	create: createAction(routes.dashboard.tenants.create, async (ctx) => {
		let { db, formData, platformSession, log } = ctx;
	...
	edit: createAction(routes.dashboard.tenants.edit, async (ctx) => {
		let { db, params, platformSession, log } = ctx;
		let { id } = ds.parse(ds.object({ id: ds.string() }), params);
	...
	update: createAction(routes.dashboard.tenants.update, async (ctx) => {
		let { db, formData, params, platformSession, log } = ctx;
		let { id } = ds.parse(ds.object({ id: ds.string() }), params);
