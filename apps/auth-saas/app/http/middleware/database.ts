/**
 * Publishes the platform database as `ctx.db`, so a controller reads its dependency off
 * the context it was already given and a test hands one in by installing this middleware
...
declare module "remix/router" {
	interface RequestContext {
		/** The platform database, published by the global `database()` middleware. */
		db: DataTable;
	}
}
