...
export const maintenanceShow = createAction(routes.api.v1.maintenanceShow, async (ctx) => {
...
export const maintenanceUpdate = createAction(routes.api.v1.maintenanceUpdate, async (ctx) => {
	let { maintenanceId } = s.parse(MaintenanceIdParams, ctx.params);
	let db = getServiceContainer().get(Database);
...
export const maintenanceDestroy = createAction(routes.api.v1.maintenanceDestroy, async (ctx) => {
	let { maintenanceId } = s.parse(MaintenanceIdParams, ctx.params);
	let db = getServiceContainer().get(Database);
	let existing = await MaintenanceWindow.findByIdForTeam(db, ctx.apiTeam.id, maintenanceId);
	if (!existing) return apiError("NOT_FOUND", "Maintenance window not found", NotFound);
...
export const maintenanceEnd = createAction(routes.api.v1.maintenanceEnd, async (ctx) => {
	let { maintenanceId } = s.parse(MaintenanceIdParams, ctx.params);
	let db = getServiceContainer().get(Database);
	let existing = await MaintenanceWindow.findByIdForTeam(db, ctx.apiTeam.id, maintenanceId);
	if (!existing) return apiError("NOT_FOUND", "Maintenance window not found", NotFound);
